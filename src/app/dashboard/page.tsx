...
import { Users, Building2, DollarSign, Activity, ChevronRight } from "lucide-react";
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div>
                        <div className="bg-[#1e3a5f] px-4 py-2 flex justify-between items-center">
                            <h3 className="text-sm font-semibold text-white">Recent Onboarding Activity</h3>
                            <button className="text-xs text-white/80 hover:text-white underline">View All</button>
                        </div>
                        <div className="bg-white border border-t-0 border-gray-200">
                            <table className="w-full text-left text-sm">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {[
                                        { name: "Acme Corp Inc.", broker: "Sarah Johnson", status: "Active", date: "Oct 24, 2024", color: "green" },
                                        { name: "TechFlow Solutions", broker: "Mike Peters", status: "Pending", date: "Oct 23, 2024", color: "yellow" },
                                        { name: "Global Logistics Ltd", broker: "Sarah Johnson", status: "Onboarding", date: "Oct 21, 2024", color: "blue" },
                                        { name: "Starlight Media", broker: "James Anderson", status: "Draft", date: "Oct 19, 2024", color: "gray" },
                                    ].map((row, i) => (
                                        <tr key={i} className="hover:bg-gray-50">
                                            <td className="px-6 py-3 font-medium text-gray-900">{row.name}</td>
                                            <td className="px-6 py-3 text-gray-600">{row.broker}</td>
                                            <td className="px-6 py-3">
                                                <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium bg-${row.color}-100 text-${row.color}-800`}>
                                                    {row.status}
                                                </span>
                                            </td>
                                            <td className="px-6 py-3 text-gray-500">{row.date}</td>
                                            <td className="px-6 py-3 text-right text-blue-600 hover:underline cursor-pointer">Manage</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
